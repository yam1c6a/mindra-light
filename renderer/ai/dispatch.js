    ...
    try {
      ...
      if (cmd.type === "translate") {
        if (
          ...
          typeof window.mindraTranslator.translateActivePage === "function"
        ) {
          // 言語指定がなければデフォルトは日本語
          const targetLang = cmd.targetLang || "ja";

          const res = await window.mindraTranslator.translateActivePage(
            targetLang
          );

          if (res && res.ok !== false) {
            return (res.translation || res.text || "").toString();
          }
